    ...
    def run(self):
        while str(threading.enumerate()[0]).find("started") > 0 :
            while len(zstack) > 0 and str(threading.enumerate()[0]).find("started") > 0 :
                ...
                print("zbench_time, mesure de l'overlay")
                zstart = time.time()
                cmd = "docker exec -it docker-ressenti /bin/bash /root/work/screenshot.sh  " + "about:blank" + " " + zimg
                print("zbench_time, cmd: " + cmd)
                os.system(cmd)
                zend = time.time()
                zoverlay = zend-zstart
                print("zbench_time, overlay: " + str(zoverlay))

                print("zbench_time, mesure du temps de chargement")
                zstart = time.time()
                cmd = "docker exec -it docker-ressenti /bin/bash /root/work/screenshot.sh  " + zurl + " " + zimg
                print("zbench_time, cmd: " + cmd)
                os.system(cmd)
                zend = time.time()
                zresult = (zend-zstart)-zoverlay
                print("zbench_time, zresult: " + str(zresult))
                if zresult < 0 : zresult = 0
                ztime_table[zurl] = (str(zresult), datetime.datetime.now())
